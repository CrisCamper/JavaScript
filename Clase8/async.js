...
const getFactureInfo = async (id) => {
    try{
        const idClient = facturas.find(f => f.id === id)?.clienteId;
        if (idClient){
            const clientInfo = await getClient(idClient);
        }

        const idProducts = facturas.find(f => f.id === id)?.productos;

        if (idProducts){
            forEach(idProducts){
                let value = [];
                let cost = getProduct(idProducts);
            }
        }
    }
}
